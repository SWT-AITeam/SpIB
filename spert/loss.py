...
class SpERTLoss(Loss):
    ...
    def compute(self, entity_logits, rel_logits, entity_types, rel_types, entity_sample_masks, rel_sample_masks,
        ...
        rel_sum = torch.ones(rel_sample_masks.size()).to(entity_logits.device)
        kld_rel_loss = kld_rel.sum() / rel_sum.sum()
        # torch.matmul(kld_rel,
        #                     torch.ones(rel_sample_masks.shape).T.type(torch.float32).to("cuda")).sum()/4/rel_sample_masks.sum())
        ...
        if rel_count.item() != 0:
            ...
            # joint loss
            train_loss = entity_loss + rel_loss + kld_rel_loss * float(beta)
