...
# AdamW是一个优化器类
# BertConfig是BERT模型的配置类，它包含了BERT模型的各种超参数和配置选项
...
# 将当前脚本文件所在的目录路径保存在SCRIPT_PATH变量中
...
class SpERTTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader], beta):
        args = self._args
        ...
        for epoch in range(args.epochs):
            # train epoch
            self._train_epoch(model, compute_loss, optimizer, train_dataset, updates_epoch, epoch, beta)
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
                     updates_epoch: int, epoch: int, beta):
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            # forward step
            entity_logits, rel_logits, kld_rel = model(encodings=batch['encodings'],
